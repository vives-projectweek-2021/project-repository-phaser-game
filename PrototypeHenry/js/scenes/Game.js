...
export class Game extends Phaser.Scene{
    ...
    //create powerups
    this.lowGrav = this.physics.add.group();
    this.physics.add.collider(this.lowGrav, this.platforms);

    //timer
    this.timedEvent = this.time.addEvent({ delay: Phaser.Math.Between(500, 5000), callback: this.spawn, callbackScope: this, loop: true });

    this.powerEvent = this.time.addEvent({delay: 60000, callback: this.spawn, callbackScope: this, loop: true });
